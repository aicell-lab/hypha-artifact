...
async def edit(
    ...
    """Edits an existing artifact's manifest. The new manifest is staged until committed if
        staging mode is enabled.

    Parameters:

        artifact_id: The id of the artifact to edit. It can be an uuid generated by create or edit
            function, or it can be an alias of the artifact under the current workspace.
            If you want to refer to an artifact in another workspace, you should use the full
            alias in the format of "workspace_id/alias".

        manifest: The updated manifest. Ensure the manifest follows the required schema if
            applicable (e.g., for collections).

        type: Optional. The type of the artifact. Supported values are collection, generic and
            any other custom type. By default, it's set to generic which contains fields tailored
            for displaying the artifact as cards on a webpage.

        permissions: Optional. A dictionary containing user permissions. For example {"*": "r+"}
            gives read and create access to everyone, {"@": "rw+"} allows all authenticated users
            to read/write/create, and {"user_id_1": "r+"} grants read and create permissions to a
            specific user. You can also set permissions for specific operations,
            such as {"user_id_1": ["read", "create"]}. See detailed explanation about
            permissions below.

        secrets: Optional. A dictionary containing secrets to be stored with the artifact.
            Secrets are encrypted and can only be accessed by the artifact owner or users with
            appropriate permissions. See the create function for a list of supported secrets.

        config: Optional. A dictionary containing additional configuration options for the artifact.

        version: Optional. Strict Validation Applied: Must be None, "new", or an existing
            version name from the artifact's versions array. Custom version names are NOT
            allowed during edit - they can only be specified during commit().
        ...
        Special Staging Behavior: When stage=True and version="new" are used together,
            ...
            and allows custom version names to be specified later during the commit() operation.

        stage: Optional. If True, the artifact will be edited in staging mode regardless
            of the version parameter. Default is False. When in staging mode, the artifact must
            be committed to finalize changes.
    ...
    Important Notes:
        ...
        Historical Version Editing: You can edit specific existing versions by providing
            their exact version name (e.g., version="v1"), but the version must already exist.
    """
...
async def commit(
    ...
    """Commits staged changes to an artifact,
        creating a new version or updating the existing latest version depending on the
        staging mode and version intent.

    Parameters:

        artifact_id: The id of the artifact to commit. Can be a UUID or alias.
        version: Optional custom version name. If not provided, a sequential version name
            will be generated.
    ...
    Behavior:
        ...
        File Placement:
            Files are already in their final locations (placed there during upload based on intent)
        Staging Cleanup: The staging area is cleared and metadata is finalized

    Performance Note: The commit operation is highly optimized and performs no file copying,
        making it fast even for artifacts with large files.
    """
    # TODO: Add specific prepare_params for each artifactmethod, with pre-filled and required ones
    params: dict[str, Any] = prepare_params(
        self,
        {
            "version": version,
...
async def create(
    ...
    Parameters:
        ...
        parent_id: The id of the parent collection where the artifact will be created.
            If the artifact is a top-level collection, leave this field empty or set to None.

        type: The type of the artifact. Supported values are collection, vector-collection,
            generic and any other custom type. By default, it's set to generic which contains
            fields tailored for displaying the artifact as cards on a webpage.

        manifest: The manifest of the new artifact. Ensure the manifest follows the required schema
            if applicable (e.g., for collections).

        config: Optional. A dictionary containing additional configuration options for
            the artifact (shared for both staged and committed). For collections, the config
            can contain the following special fields:

            collection_schema: Optional. A JSON schema that defines the structure of child
                artifacts in the collection. This schema is used to validate child artifacts when
                they are created or edited. If a child artifact does not conform to the schema, the
                creation or edit operation will fail.
            id_parts: Optional. A dictionary of id name parts to be used in generating the id for
                child artifacts. For example:
                {"animals": ["dog", "cat", ...], "colors": ["red", "blue", ...]}.
                This can be used for creating child artifacts with auto-generated ids based on the
                id parts. For example, when calling create, you can specify the alias as
                my-pet-{colors}-{animals}, and the id will be generated based on the id parts, e.g.,
                my-pet-red-dog.
            permissions: Optional. A dictionary containing user permissions. For example
                {"*": "r+"} gives read and create access to everyone, {"@": "rw+"} allows all
                authenticated users to read/write/create, and {"user_id_1": "r+"} grants read and
                create permissions to a specific user. You can also set permissions for specific
                operations, such as {"user_id_1": ["read", "create"]}. See detailed explanation
                about permissions below.
            list_fields: Optional. A list of fields to be collected when calling list function.
                By default, it collects all fields in the artifacts. If you want to collect only
                specific fields, you can set this field to a list of field names,
                e.g. ["manifest", "download_count"].
            publish_to: Optional. A string specifying the target platform to publish the artifact.
                Supported values are zenodo and sandbox_zenodo. If set, the artifact will be
                published to the specified platform. The artifact must have a valid Zenodo metadata
        ...
        secrets: Optional. A dictionary containing secrets to be stored with the artifact.
            ...
            S3_PUBLIC_ENDPOINT_URL: The public endpoint URL of the S3 storage for the artifact.
                If the S3 server is not public, you can set this to the public endpoint URL. Default: None.

        overwrite: Optional. A boolean flag to overwrite the existing artifact with the same alias.
            Default is False.
...
async def delete(
    ...
    Parameters:
        ...
        artifact_id: The id of the artifact to delete. It can be an uuid generated by create or edit
            function, or it can be an alias of the artifact under the current workspace.
            If you want to refer to an artifact in another workspace, you should use the full alias
            in the format of "workspace_id/alias".
        delete_files: Optional. A boolean flag to delete all files associated with the artifact.
            Default is False.
    ...
    Warning: If delete_files is set to True, recursive must be set to True, all
        child artifacts will be deleted, and all files associated with the child artifacts
        will be permanently deleted from the S3 storage. This operation is irreversible.
    """
    params: dict[str, Any] = prepare_params(
        self,
        {
